...
const getAllpurchaseAndPendingCoursesFromDb = async (
  filters: IPurchaseCourseFilters,
  paginationOptions: IPaginationOption,
): Promise<IGenericResponse<IPurchaseCourse[]>> => {
  //****************search and filters start************/
  ...
  const result = await PendingPurchaseCourse.find(whereConditions)
    .sort(sortConditions)
    .skip(Number(skip))
    .limit(Number(limit))
    .populate('course')
  ...
  const total = await PendingPurchaseCourse.countDocuments(whereConditions);
  return {
    meta: {
      page,
      limit,
